...
{"B:BUCURESTI SECTORUL 6", 0.48},
        ...
        public List<Scoring> GetScoring()
        {
            ...
            string tLocalitate = "";
            string tRaspuns37 = "", tRaspuns38 = "", tRaspuns39 = "", tRaspuns40 = "", tRaspuns41 = "", tRaspuns46 = "", tRaspuns47 = "";
            foreach (Unitate MyUnitate in AllUnitati)
            {
                if (MyUnitate.Moderator != "1")
                {
                    if (MyUnitate.Statut == "PJ")
                    {
                        UnitateIntrebare action37 = AllUnitateIntrebari.Where(x => x.UnitateId == MyUnitate.Id && x.IntrebareId == 37).FirstOrDefault();
                        UnitateIntrebare action38 = AllUnitateIntrebari.Where(x => x.UnitateId == MyUnitate.Id && x.IntrebareId == 38).FirstOrDefault();
                        UnitateIntrebare action39 = AllUnitateIntrebari.Where(x => x.UnitateId == MyUnitate.Id && x.IntrebareId == 39).FirstOrDefault();
                        UnitateIntrebare action40 = AllUnitateIntrebari.Where(x => x.UnitateId == MyUnitate.Id && x.IntrebareId == 40).FirstOrDefault();
                        UnitateIntrebare action41 = AllUnitateIntrebari.Where(x => x.UnitateId == MyUnitate.Id && x.IntrebareId == 41).FirstOrDefault();
                        ...
                        if (action37 != null) tRaspuns37 = action37.Raspuns;
                        else tRaspuns37 = null;
                        if (action38 != null) tRaspuns38 = action38.Raspuns;
                        else tRaspuns38 = null;
                        if (action39 != null) tRaspuns39 = action39.Raspuns;
                        else tRaspuns39 = null;
                    ...
                    MyScoring.Raspuns37 = tRaspuns37;
                    MyScoring.Raspuns38 = tRaspuns38;
                    MyScoring.Raspuns39 = tRaspuns39;
                    MyScoring.Raspuns40 = tRaspuns40;
                    MyScoring.Raspuns41 = tRaspuns41;
                    MyScoring.Raspuns46 = tRaspuns46;
                    MyScoring.Raspuns47 = tRaspuns47;



                    MyScoring.Sirues = TempScoala.Sirues;
                    MyScoring.Mediu = TempScoala.Mediu;
                    MyScoring.Msanitar = TempScoala.Msanitar;
                    MyScoring.Mscolar = TempScoala.Mscolar;
                    MyScoring.Romani = TempScoala.Romani;
                    MyScoring.Finantari = TempScoala.Finantari;
                    MyScoring.Plan = TempScoala.Plan;
                    MyScoring.Expert = TempScoala.Expert;


                    MyScoring.LocalitateS = tLocalitate;

                    if (TempScoala.TotalC != null && TempScoala.TotalC != "0")
                    {
                        ...
                        if (TempScoala.RepetentiC != null) MyScoring.ScorE1 = Math.Round(Double.Parse(TempScoala.RepetentiC) / Double.Parse(TempScoala.TotalC) * 100, 2, MidpointRounding.AwayFromZero);
                        if (TempScoala.OnlineC != null) MyScoring.ScorE2 = Math.Round(Double.Parse(TempScoala.OnlineC) / Double.Parse(TempScoala.TotalC) * 100, 2, MidpointRounding.AwayFromZero);
                        if (TempScoala.RemedialaC != null) MyScoring.ScorE3 = Math.Round(Double.Parse(TempScoala.RemedialaC) / Double.Parse(TempScoala.TotalC) * 100, 2, MidpointRounding.AwayFromZero);
                    }
                    ...
                    if (MyUnitate.TotalC != null && MyUnitate.TotalC != "0")
                    {
                    ...
                    {
                        if (Int32.Parse(TempScoala.RomiC) > 4) MyScoring.ScorA2 = Math.Round(Math.Abs(MyScoring.RomiUnitateP - MyScoring.ScorA1) / 10, 2, MidpointRounding.AwayFromZero); else MyScoring.ScorA2 = 0;
                        if (Int32.Parse(TempScoala.DizabilitatiC) > 4) MyScoring.ScorB2 = Math.Round(Math.Abs(MyScoring.DizabilitatiUnitateP - MyScoring.ScorB1) / 10, 2, MidpointRounding.AwayFromZero); else MyScoring.ScorB2 = 0;
                        if (Int32.Parse(TempScoala.ParintiC) > 4) MyScoring.ScorC2 = Math.Round(Math.Abs(MyScoring.ParintiUnitateP - MyScoring.ScorC1) / 10, 2, MidpointRounding.AwayFromZero); else MyScoring.ScorC2 = 0;
                        if (Int32.Parse(TempScoala.BurseC) > 4) MyScoring.ScorC7 = Math.Round(Math.Abs(MyScoring.BurseUnitateP - MyScoring.ScorC6) / 10, 2, MidpointRounding.AwayFromZero); else MyScoring.ScorC7 = 0;
                    }
                    ...
                    {
                        if (MyScoring.TotalCladire[i] == 0)
                        {
                            MyScoring.RomiScorCladire[i] = 0;
                            MyScoring.DizabilitatiScorCladire[i] = 0;
                            MyScoring.ParintiScorCladire[i] = 0;
                            MyScoring.BurseScorCladire[i] = 0;
                        }
                        else
                        {
                            MyScoring.RomiScorCladire[i] = Math.Round(Math.Abs((MyScoring.RomiCladire[i] / MyScoring.TotalCladire[i] * 100) - MyScoring.RomiUnitateP) / 10, 2, MidpointRounding.AwayFromZero); //if romicladire[i] > 4 fa asta, altfel myscoring.romiscorcladire[i] = 0
                    ...
                    foreach (Clasa MyClasa in MyClase)
                    {
                        if (MyClasa.Moderator != "1")
                        {
                                ...
                                MyClasa.CladireC != null)
                            {
                                ...
                                {
                                    ProcentRomiClasa = 0;
                                    ProcentDizabilitatiClasa = 0;
                                    ProcentParintiClasa = 0;
                                    ProcentBurseClasa = 0;
                                    ProcentRepetentiClasa = 0;
                                    ProcentOnlineClasa = 0;
                                    ProcentRemedialaClasa = 0;
                                }

                                if (MyClasa.Cifra != "Mixt")
                                {
                                    if (MyScoring.TotalNivel[Int32.Parse(MyClasa.Cifra)] == 0)
                                    {
                                        MyScoring.RomiScorNivel.Add(0);
                                        MyScoring.DizabilitatiScorNivel.Add(0);
                                        MyScoring.ParintiScorNivel.Add(0);
                                        MyScoring.BurseScorNivel.Add(0);
                                        MyScoring.RepetentiScorNivel.Add(0);
                                        MyScoring.OnlineScorNivel.Add(0);
                                        MyScoring.RemedialaScorNivel.Add(0);
                                    }
                                    else
                                    {
                                        if (MyScoring.RomiNivel[Int32.Parse(MyClasa.Cifra)] > 4) MyScoring.RomiScorNivel.Add(Math.Round(Math.Abs(ProcentRomiClasa - (MyScoring.RomiNivel[Int32.Parse(MyClasa.Cifra)] / MyScoring.TotalNivel[Int32.Parse(MyClasa.Cifra)] * 100)) / 10, 2, MidpointRounding.AwayFromZero)); else MyScoring.RomiScorNivel.Add(0);
                                        if (MyScoring.DizabilitatiNivel[Int32.Parse(MyClasa.Cifra)] > 4) MyScoring.DizabilitatiScorNivel.Add(Math.Round(Math.Abs(ProcentDizabilitatiClasa - (MyScoring.DizabilitatiNivel[Int32.Parse(MyClasa.Cifra)] / MyScoring.TotalNivel[Int32.Parse(MyClasa.Cifra)] * 100)) / 10, 2, MidpointRounding.AwayFromZero)); else MyScoring.DizabilitatiScorNivel.Add(0);
                                        if (MyScoring.ParintiNivel[Int32.Parse(MyClasa.Cifra)] > 4) MyScoring.ParintiScorNivel.Add(Math.Round(Math.Abs(ProcentParintiClasa - (MyScoring.ParintiNivel[Int32.Parse(MyClasa.Cifra)] / MyScoring.TotalNivel[Int32.Parse(MyClasa.Cifra)] * 100)) / 10, 2, MidpointRounding.AwayFromZero)); else MyScoring.ParintiScorNivel.Add(0);
                                        if (MyScoring.BurseNivel[Int32.Parse(MyClasa.Cifra)] > 4) MyScoring.BurseScorNivel.Add(Math.Round(Math.Abs(ProcentBurseClasa - (MyScoring.BurseNivel[Int32.Parse(MyClasa.Cifra)] / MyScoring.TotalNivel[Int32.Parse(MyClasa.Cifra)] * 100)) / 10, 2, MidpointRounding.AwayFromZero)); else MyScoring.BurseScorNivel.Add(0);
                                        if (MyScoring.RepetentiNivel[Int32.Parse(MyClasa.Cifra)] > 4) MyScoring.RepetentiScorNivel.Add(Math.Round(Math.Abs(ProcentRepetentiClasa - (MyScoring.RepetentiNivel[Int32.Parse(MyClasa.Cifra)] / MyScoring.TotalNivel[Int32.Parse(MyClasa.Cifra)] * 100)) / 10, 2, MidpointRounding.AwayFromZero)); else MyScoring.RepetentiScorNivel.Add(0);
                                        if (MyScoring.OnlineNivel[Int32.Parse(MyClasa.Cifra)] > 4) MyScoring.OnlineScorNivel.Add(Math.Round(Math.Abs(ProcentOnlineClasa - (MyScoring.OnlineNivel[Int32.Parse(MyClasa.Cifra)] / MyScoring.TotalNivel[Int32.Parse(MyClasa.Cifra)] * 100)) / 10, 2, MidpointRounding.AwayFromZero)); else MyScoring.OnlineScorNivel.Add(0);
                                        if (MyScoring.RemedialaNivel[Int32.Parse(MyClasa.Cifra)] > 4) MyScoring.RemedialaScorNivel.Add(Math.Round(Math.Abs(ProcentRemedialaClasa - (MyScoring.RemedialaNivel[Int32.Parse(MyClasa.Cifra)] / MyScoring.TotalNivel[Int32.Parse(MyClasa.Cifra)] * 100)) / 10, 2, MidpointRounding.AwayFromZero)); else MyScoring.RemedialaScorNivel.Add(0);
                                    }
                                }

                                if (MyClasa.Total2bC == "0")
                                {
                                    MyScoring.RomiScor2b.Add(0);
                                    MyScoring.DizabilitatiScor2b.Add(0);
                                    MyScoring.ParintiScor2b.Add(0);
                                    MyScoring.BurseScor2b.Add(0);
                                }
                                else
                                {
                                    if ((Double.Parse(MyClasa.Romi2bC) / Double.Parse(MyClasa.Total2bC) * 100) < ProcentRomiClasa) MyScoring.RomiScor2b.Add(0);
                                    else MyScoring.RomiScor2b.Add(Math.Round(Math.Abs((Double.Parse(MyClasa.Romi2bC) / Double.Parse(MyClasa.Total2bC) * 100) - ProcentRomiClasa) / 10, 2, MidpointRounding.AwayFromZero));

                                    if ((Double.Parse(MyClasa.Dizabilitati2bC) / Double.Parse(MyClasa.Total2bC) * 100) < ProcentDizabilitatiClasa) MyScoring.DizabilitatiScor2b.Add(0);
                                    else MyScoring.DizabilitatiScor2b.Add(Math.Round(Math.Abs((Double.Parse(MyClasa.Dizabilitati2bC) / Double.Parse(MyClasa.Total2bC) * 100) - ProcentDizabilitatiClasa) / 10, 2, MidpointRounding.AwayFromZero));

                                    if ((Double.Parse(MyClasa.Parinti2bC) / Double.Parse(MyClasa.Total2bC) * 100) < ProcentParintiClasa) MyScoring.ParintiScor2b.Add(0);
                                    else MyScoring.ParintiScor2b.Add(Math.Round(Math.Abs((Double.Parse(MyClasa.Parinti2bC) / Double.Parse(MyClasa.Total2bC) * 100) - ProcentParintiClasa) / 10, 2, MidpointRounding.AwayFromZero));

                                    if ((Double.Parse(MyClasa.Burse2bC) / Double.Parse(MyClasa.Total2bC) * 100) < ProcentBurseClasa) MyScoring.BurseScor2b.Add(0);
                                    else MyScoring.BurseScor2b.Add(Math.Round(Math.Abs((Double.Parse(MyClasa.Burse2bC) / Double.Parse(MyClasa.Total2bC) * 100) - ProcentBurseClasa) / 10, 2, MidpointRounding.AwayFromZero));
                                }
                            }
